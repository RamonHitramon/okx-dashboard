...
class OKXDataFetcher {
    ...
    mergeData(instruments, tickers) {
        ...
        const mergedData = instruments.map(instrument => {
            ...
            return {
                ...
                ts: ticker ? ticker.ts : '',
                // Вычисляем минимальный размер ордера в USD
                minOrderValueUSD: this.calculateMinOrderValueUSD(instrument, ticker)
            };
        });
    ...
    // Вычисление минимального размера ордера в USD
    calculateMinOrderValueUSD(instrument, ticker) {
        try {
            const minSz = parseFloat(instrument.minSz || '0');
            const last = parseFloat(ticker?.last || '0');
            
            if (minSz > 0 && last > 0) {
                return (minSz * last).toFixed(2);
            }
    ...
    async run() {
        try {
            ...
            // Объединяем данные
            const mergedData = this.mergeData(instruments, tickers);
            
            // Сохраняем полный файл
            this.saveToCSV(mergedData, 'okx_all_pairs_complete.csv');
            
            // Сохраняем только активные инструменты (с данными тикеров)
            const activePairs = mergedData.filter(item => item.last && item.last !== '');
            this.saveToCSV(activePairs, 'okx_active_pairs_complete.csv');
            
            // Сохраняем только SWAP инструменты
            const swapPairs = mergedData.filter(item => item.instType === 'SWAP');
            this.saveToCSV(swapPairs, 'okx_swap_pairs_complete.csv');
            
            // Сохраняем только SPOT инструменты
            const spotPairs = mergedData.filter(item => item.instType === 'SPOT');
            this.saveToCSV(spotPairs, 'okx_spot_pairs_complete.csv');
            
            // Создаем упрощенный файл с основными полями
            const simplifiedData = mergedData.map(item => ({
                instId: item.instId,
                ...
                last: item.last,
                vol24h: item.vol24h,
                minOrderValueUSD: item.minOrderValueUSD
            }));
            
            this.saveToCSV(simplifiedData, 'okx_pairs_simplified.csv');
            
            console.log('\n✅ Загрузка завершена!');
            console.log('📁 Созданные файлы:');
            console.log('- okx_all_pairs_complete.csv (все инструменты с полными данными)');
            console.log('- okx_active_pairs_complete.csv (активные инструменты)');
            console.log('- okx_swap_pairs_complete.csv (только SWAP)');
            console.log('- okx_spot_pairs_complete.csv (только SPOT)');
            console.log('- okx_pairs_simplified.csv (упрощенная версия)');
